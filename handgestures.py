...
# Constants
KNOWN_WIDTH = 8.0  # in cm
FOCAL_LENGTH = 500  # adjust as needed
ALERT_DISTANCE_THRESHOLD = 20  # cm
ALERT_COOLDOWN = 2  # seconds between beeps
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Trigger beep if hand is close
            current_time = time.time()
            if distance < ALERT_DISTANCE_THRESHOLD and (current_time - last_alert_time > ALERT_COOLDOWN):
                beep_alert()
